...
class FinancialTransactionResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        ...
        return [
            ...
            'reason' => $transaction->reason, 'description' => $transaction->description, 'reference_number' => $transaction->reference_number, 'occurred_at' => $transaction->occurred_at->toISOString(),
            'occurred_timezone' => $transaction->occurred_timezone, 'posted_at' => $transaction->posted_at?->toISOString(), 'reversed_at' => $transaction->reversed_at?->toISOString(), 'cancelled_at' => $transaction->cancelled_at?->toISOString(),
            'journal_entry_id' => $transaction->journal_entry_id, 'reversal_of_id' => $transaction->reversal_of_id, 'correction_of_id' => $transaction->correction_of_id, 'duplicate_replaced_by_id' => $transaction->duplicate_replaced_by_id,
